...
def filter_cloud_by_minh(cloud):
    ...
    for p in cloud:
        ...
        point_row_col_ori.append((row, col))
    ...
    return pcl_data, point_row_col_f, point_row_col_ori, abs(max_img_f-min_img_f)
...
def detect_narrow(img):
    ...
    # dilation = cv2.dilate(img, kernelhdiff_img, slope_img, roughness_img, obs_rgb, point_row_col_dilate,iterations = 1)
    # dilation_diff = cv2.dilate(img, khdiff_img, slope_img, roughness_img, obs_rgb, point_row_colernel_diff,iterations = 1)
    # erosion = cv2.erode(dilation, kerhdiff_img, slope_img, roughness_img, obs_rgb, point_row_colnel_dilate,iterations = 1)
...
def get_slope_roughness_img(cloud_filtered, point_row_col, normal_big, roughness):
    ################################################################################################
    ...
    if normal_big == [] or roughness == []:
        ...
        print('skip normal')
        normal_big = np.asarray(normal_big)
        cloud_roughness = abs(np.asarray(roughness))

    cloud_slope = 1 - abs(normal_big)
    ...
    for slope, roughness, (row, col)in zip(cloud_slope, cloud_roughness, point_row_col):
        if np.isnan(slope[2]) or np.isnan(roughness[2]):
            continue
        # min_value = min_img[row, col]
        # print(n_b[2], n_s[2])
        ...
        sumh_slope[row, col] += slope[2]
        sumh_roughness[row, col] += roughness[2]
        count_img[row, col] += 1
...
def compute_geometric_features(map_cloud_pcl, map_w, map_b, map_resolution, normal_big=[], roughness=[]):
    global Map_W, Map_B, Map_resolution
    hdiff_img, slope_img, roughness_img, obs_rgb, point_row_col = [], [], [], [], []

    Map_W = map_w 
    Map_B = map_b 
    Map_resolution = map_resolution

    cloud_filtered, point_row_col, point_row_col_ori, hdiff_img = filter_cloud_by_minh(map_cloud_pcl)
    print('         filtered by local min')
    ...
    # compute slope and roughness
    ###############################################
    slope_img, roughness_img = get_slope_roughness_img(cloud_filtered, point_row_col, normal_big, roughness)

    return hdiff_img, slope_img, roughness_img, obs_rgb, point_row_col, cloud_filtered.to_array()